...
from agent.schemas import ContentRequest, ContentOutput, SEOData, ContentVariation
from agent.prompts import (
    SOCIAL_MEDIA_PROMPT,
    EMAIL_CAMPAIGN_PROMPT,
    PROMOTIONAL_MESSAGE_PROMPT
)
from agent.tools import PLATFORM_RULES
from knowledge_base.content_kb import retrieve_brand_knowledge
...
def parse_variations(text: str) -> list[ContentVariation]:
    variations = []
...
def run_content_agent(request: ContentRequest) -> ContentOutput:
    ...
    platform_rules_str = "\n".join([
        f"- {k}: {v}" for k, v in platform_rules.items()
    ]) if platform_rules else "No specific platform rules."

    # Step 2: RAG retrieval
    print(f"\n🔍 Retrieving brand knowledge for: {request.brand_name}...")
    ...
    # Step 5: Generate
    print(f"⚙️  Generating {request.content_type}...")
    ...
    if request.content_type == "social_media_post":
        hashtags   = [w.strip('.,!?"\'') for w in generated_text.split() if w.startswith("#")]
